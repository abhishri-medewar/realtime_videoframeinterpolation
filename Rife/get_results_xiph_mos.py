...
parser.add_argument('--data_root',, type=str, default='/home/abhishri/realtime_vfi/sample_mos_data/xiph')
...
for strFile in video_filename:
    ...
    for intFrame in range(2, 99, 2): 
        if os.path.exists(args.data_root + '/' + strFile + '/' + strFile + '-' + str(intFrame).zfill(3) + '.png') == True:       
            img0 = cv2.imread(args.data_root + '/' + strFile + '/' + strFile + '-' + str(intFrame - 1).zfill(3) + '.png', cv2.IMREAD_UNCHANGED)
            img1 = cv2.imread(args.data_root + '/' + strFile + '/' + strFile + '-' + str(intFrame + 1).zfill(3) + '.png', cv2.IMREAD_UNCHANGED)
            gt = cv2.imread(args.data_root + '/' + strFile + '/' + strFile + '-' + str(intFrame).zfill(3) + '.png', cv2.IMREAD_UNCHANGED)
